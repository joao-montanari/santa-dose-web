...
import { addDaysBillsValue, createMonthBillsValue, getDaysBillsValue } from "@Api/services/billsMes";
import getPhotoUser from "@Api/services/getPhotoUser";

const Bills = () =>{
    const navigate = useNavigate()
    const [product, setProduct] = useState({tipo: '', dia: 0, motivo: ''});
    ...
    async function handleSubmitBillsDay() {
        if (!product.tipo) {
            setNote({
        ...
        try {
            ...
            console.log("Dados enviados: ", registro)
                ...
                const response = await addDaysBillsValue(registro);
                if (response.error) {
                    setNote({
    ...
    async function handleSubmitBillsValues() {
        if(!product.tipo) {
            setNote({
            ...
            try{
                const response = await createMonthBillsValue(submitData); // chamando para enviar os dados
                if (response.error){
                    setNote({
                    ...
                    localStorage.setItem("product-operation", "Valores do mês enviados!");
                    navigate("/financial/monthly-bills");
                }
            }catch(error) {
                console.error("Erro ao enviar valores", error);
                setNote({
    ...
    async function getDailyBills(){
        ...
        setLoading(true)
        try{
            const response = await getDaysBillsValue(product.tipo);
            const novosSales: { day: string; value: string; reason: string}[] = [];
    ...
    return(
        <div id="product-list-main">
            <div id="product-list-header">
                <Title
                title='Boletos'
                subTitle='Veja os valores dos boletos diários da Adega Santa Dose'
                />
            ...
            <TableSales
                title="Boletos"
                dayColumnTitle="Dias"
                salesColumTitle="Valores Boletos"
                totalLabel="Total Do Mês"
                daysInMonth={31}
                nameAccount="Nome Boleto"
            ...
            <div id="buttons-align">
                <div id="buttons-justify">
                    <Button title="Pegar Valores De Boletos Salvos No Mês" onClick={getDailyBills} />
                    <Button title="Salvar Valores De Boletos Nos Dias" onClick={handleSubmitBillsDay}/>
                    <Button title="Enviar Valores De Boletos Do Mês" onClick={handleSubmitBillsValues}/>
                </div>
            </div>
