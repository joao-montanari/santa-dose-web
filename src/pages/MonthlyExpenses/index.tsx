...
const monthlyExpenses = () =>{
    ...
    async function handleClarValues() {
        if(!product.tipo) {
            ...
            try{
                const response = await deleteMonthExpensesValue(product.tipo); // chamando para enviar os dados
                if (response.error){
                    setNote({
    ...
    async function monthValues() {
        setLoading(true);
        const data = await getExpensesMonthValue();
                ...
                <div id="month-justify-place">
                    <div id="month-sells-background">
                        <div id="month-title-content">
                            <h1>Gastos Aleatórios Mensais</h1>
                        </div>
