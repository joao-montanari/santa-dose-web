...
import { deletCardMonthExpensesValue, getCardExpensesMonthValue } from "@Api/services/expensesCard";
import getPhotoUser from "@Api/services/getPhotoUser";

const ExpensesCardMonthly = () => {
    ...
    async function handleClarValues() {
        if(!product.tipo) {
            ...
            try{
                const response = await deletCardMonthExpensesValue(product.tipo); // chamando para enviar os dados
                if (response.error){
                    setNote({
    ...
    async function monthValues() {
        setLoading(true);
        const data = await getCardExpensesMonthValue();
    ...
    return(
            <>
                <div id="month-sells-header">
                    <Title 
                        title="Valor Mensáis Pagos De Cartão"
                        subTitle="Veja o total de pagamento no cartão da Adega"/>
                ...
                <div id="month-justify-place">
                    <div id="month-sells-background">
                        <div id="month-title-content">
                            <h1>Total pago no cartão mensalmente</h1>
                        </div>
...
export default ExpensesCardMonthly